...
Install from github with pip::
    ...
    pip install -e git+https://github.com/akhanf/preprocgre_bruker#egg=preprocgre_bruker
...
Do a dry-run first (``-n``) and simply print (``-p``) what would be run::
    ...
    preprocgre_bruker /path/to/bids/dir /path/to/output/dir participant -np

Run the app, using all cores::
    
    preprocgre_bruker /path/to/bids/dir /path/to/output/dir participant --cores all
...
To generate a report, run::
    ...
    preprocgre_bruker /path/to/bids/dir /path/to/output/dir participant --report
...
Here are some instructions to get your python environment set-up on graham to run preprocgre_bruker:

#. Create a virtualenv and activate it::

      cd $SCRATCH
      module load python/3
      virtualenv venv_preprocgre_bruker
      source venv_preprocgre_bruker/bin/activate
...
Scaling up to even more subjects (uses group-components to bundle multiple subjects in each job), 1 32core job for N subjects (e.g. 10)::
    ...
    preprocgre_bruker bids_dir out_dir participant  --profile cc-slurm --group-components subj=10
